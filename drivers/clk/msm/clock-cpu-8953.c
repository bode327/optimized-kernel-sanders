...
ssize_t get_Voltages(char *buf)
{
	...
	for (i = 1; i < a53_pwr_clk.c.num_fmax; i++) {
		...
		if (uv < 0) return 0;
		count += sprintf(buf + count, "Little_Volt-max:%lumhz: %d mV\n",
	...
	//Floor
		uv = cpr_regulator_get_floor_voltage(
					...
					a53_pwr_clk.c.vdd_class->vdd_uv[i]);
		if (uv < 0) return 0;
		count += sprintf(buf + count, "Little_Volt-min:%lumhz: %d mV\n",
					a53_pwr_clk.c.fmax[i] / 1000000,
					uv / 1000);
	//current
		uv = cpr_regulator_get_last_voltage(
					...
					a53_pwr_clk.c.vdd_class->vdd_uv[i]);
		if (uv < 0) return 0;
		count += sprintf(buf + count, "Little_Volt-current:%lumhz: %d mV\n",
					a53_pwr_clk.c.fmax[i] / 1000000,
					uv / 1000);
	}
	//Big ceiling
	for (i = 1; i < a53_perf_clk.c.num_fmax; i++) {
		...
		if (uv < 0)
			return 0;
		count += sprintf(buf + count, "Big_Volt-max:%lumhz: %d mV\n",
	...
	//floor
		...
		if (uv < 0)
			return 0;
		count += sprintf(buf + count, "Big_Volt-min:%lumhz: %d mV\n",
					a53_perf_clk.c.fmax[i] / 1000000,
					uv / 1000);
	//current
		uv = cpr_regulator_get_last_voltage(
		...
		if (uv < 0)
			return 0;
		count += sprintf(buf + count, "Big_Volt-current:%lumhz: %d mV\n",
					a53_perf_clk.c.fmax[i] / 1000000,
					uv / 1000);
	}
	return count;
